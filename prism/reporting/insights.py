...
def generate_observations(df: pd.DataFrame) -> list[str]:
    ...
    if not products.empty:
        ...
        observations.append(
            f"{products.index[0]} leads product revenue and contributes {share:.1f}% "
            "of the selected total."
        )
    ...
    if len(monthly) >= 2 and monthly.iloc[-2] != 0:
        ...
        observations.append(
            f"Revenue {direction} by {abs(growth):.1f}% in the latest month versus "
            "the previous month."
        )
    cancellation_rate = df["order_status"].astype(str).str.lower().eq("cancelled").mean() * 100
    if cancellation_rate > 5:
        observations.append(
            f"Cancellation rate is {cancellation_rate:.1f}%; investigate product, region, "
            "and fulfilment drivers."
        )
    return observations
